...
void CoreOption::change(int by, int doSave) {
    ...
    printfv(10, "changed option `%s' to `%s'\n", name(), entries[value]->name);
...
void CoreOption::change(const char * to, int doSave) {
    ...
    if( (to == NULL) || (to[0] == '\0') ) {
	printfv(10, "    changing option `%s' to a random value.\n", name(), to);
	value = Random(getNEntries());
	change(0,0);
	return;
    }

    printfv(10, "    changing option `%s' to `%s'.\n", name(), to);
    ...
    static const char * Loff[]  = {
	"no-l:", "no-lock:", "no-locked:",    "nol:", "nolock:", "nolocked:",
	...
	printfv(2, "Unknown entry `%s' for option `%s'\n", to, name());
...
//
void CoreOption::add(CoreOptionEntry * entry) {
    entries.add(entry);
}
...
int CoreOption::hasExtension(const char * name, const char * extension) {
    const char * pos = strstr(name, extension);
    if ( pos == NULL)
	return 0;
...
CoreOptionEntry* CoreOption::load(const char * name, 
				  char * total_name, 
				  const char * dir,
...
void CoreOption::loadDir(const char * dir,
	...
	while( (entry = readdir(directory))	 != NULL) {
	    ...
	    *strstr(feat_name, extension) = '\0';
		...
		if( (fe=load(feat_name, total_name, dir, loader)) != NULL)
		    add(fe);
...
int CoreOption::load(const char * searchPath[], 
		     const char * extraPath, const char * extension,
		     CoreOptionEntry * (* loader)(FILE *, 
						  const char *, 
						  const char *,
						  const char *) ) {
