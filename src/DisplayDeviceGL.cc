...
/* GL_EXT_paletted_texture not available on this platform (e.g. macOS) */
#endif
...
public:
	...
	if( (strcasecmp("nice", to) == 0) ||
	    (strcasecmp("nicest", to) == 0) )
	    value = 1;
	else if( (strcasecmp("fast", to) == 0) ||
		 (strcasecmp("fastest", to) == 0) )
	    value = 0;
	else
...
public:
	...
	if( strncasecmp(to, "flat", 4) == 0)
	    ...
	    value = 1;
	else OptionOnOff::change(to);
    }
    virtual const char * text() const {
	return value ? "smooth" : "flat";
    }
} glShade;
...
public:
    ...
    virtual void printString(int x, int y, const char * text, int color, int len, 
			     int noDarken) {
	...
	glReadPixels(0,0, disp_size.x, disp_size.y,
	...
	s = disp_size.x; fwrite(&s, 2, 1, file);
	s = disp_size.y; fwrite(&s, 2, 1, file);
