...
public:
    ...
    OptionS() : OptionInt("mesh-size", 32, 128, 4) {
	setH();
    }
...
int screen_plate();		int screen_2plates();		int screen_plane();
int screen_2planes();		int screen_wave1();		int screen_wave();
int screen_height();		int screen_cross();		int screen_dplane();
int screen_cwave();		int screen_nearRing();		int screen_tube();
int screen_sheight();		int screen_2dbig();		int screen_2dquad();
int screen_sphere();		int screen_cones();
...
static unsigned char rgba_pal[6][256][4];
...
#elif defined(glColorTableEXT) || (!defined(__APPLE__))
...
#else
    /* macOS: no paletted textures, upload as RGBA */
    {
	unsigned char rgbaBuf[BUFF_WIDTH * BUFF_HEIGHT * 4];
	unsigned char *src = CthughaBuffer::buffers[P].passiveBuffer;
	for (int ii = 0; ii < BUFF_WIDTH * BUFF_HEIGHT; ii++) {
	    unsigned char idx = src[ii];
	    rgbaBuf[ii*4+0] = rgba_pal[P][idx][0];
	    rgbaBuf[ii*4+1] = rgba_pal[P][idx][1];
	    rgbaBuf[ii*4+2] = rgba_pal[P][idx][2];
	    rgbaBuf[ii*4+3] = rgba_pal[P][idx][3];
	}
	if( textureQuality.needMipmaps() ) {
	    gluBuild2DMipmaps(GL_TEXTURE_2D, GL_RGBA,
			      BUFF_WIDTH, BUFF_HEIGHT,
			      GL_RGBA, GL_UNSIGNED_BYTE, rgbaBuf);
	} else {
...
void plane(int tex, GLfloat pos) {
    ...
    glBegin( GL_POLYGON);
    glTexCoord2f(0.0, 0.0 + s); glVertex3f(-3.0, pos, 0.0);
    glTexCoord2f(1.0, 0.0 + s); glVertex3f( 3.0, pos, 0.0);
    glTexCoord2f(1.0, 3.0 + s); glVertex3f( 4.0,   0, 5.0);
    glTexCoord2f(0.0, 3.0 + s); glVertex3f(-4.0,   0, 5.0);
    glEnd();
}
...
void wave(GLfloat pos[3], GLfloat normal[3], double alpha) {
    pos[2] = soundAnalyze.intensity * 
	sin(2.0*M_PI*pos[0] + alpha) * sin(2.0*M_PI*pos[1] + alpha);
    pos[0] = -1.0 + pos[0] * 2.0;
    pos[1] = -1.0 + pos[1] * 2.0;

    normal[0] = soundAnalyze.intensity * 
	cos(2.0*M_PI*pos[0] + alpha) * sin(2.0*M_PI*pos[1] + alpha);
    normal[1] = soundAnalyze.intensity * 
	sin(2.0*M_PI*pos[0] + alpha) * cos(2.0*M_PI*pos[1] + alpha);
}
int screen_wave() {
