...
void Interface::display() {
    ...
    if(title) {
	line = displayDevice->print(title, 0, 'l', 
				    (sel == -1) ? TEXT_COLOR_HIGHLIGHT : TEXT_COLOR_NORMAL);
	line = displayDevice->print("---------------------------------------------", line, 'l', 
				    TEXT_COLOR_NORMAL);
    }
    ...
    if( saveToHot) {
	displayDevice->print("save to hotkey (press 0..9)", 
			     text_size.y - (showStatus ? 2 : 1), 
			     'l', TEXT_COLOR_NORMAL);
    }
...
InterfaceElementCoreOption::InterfaceElementCoreOption(const char * t, CoreOption * o, 
					      int i1, int i2, int i3) :
...
public:
    InterfaceMain() : Interface("main", NULL, NULL) {
	extraKeymap[0] = '\0';
    }
...
ACTION(setExtraKeymap) {
    strncpy(interfaceMain.extraKeymap, p, 512);
}
...
public:
    InterfaceCoreOption() : Interface("CoreOptions", "Core Options", NULL) {
	...
	elements[0]  = new InterfaceElementOption    ("Current Buffer (<,>)  : %s", &CthughaBuffer::nCurrent);

{
	...
	elements[3]  = new InterfaceElementCoreOption("General Flame (g)     : %s", &b.flameGeneral);
	elements[4]  = new InterfaceElementCoreOption("Border (=)            : %s", &b.border);
	elements[5]  = new InterfaceElementCoreOption("Translate (t,T)       : %s", &b.translate);
	elements[6]  = new InterfaceElementCoreOption("Wave (w)              : %s", &b.wave);
	elements[7]  = new InterfaceElementCoreOption("Sound Processing (m,M): %s", &b.soundProcess);
	elements[8]  = new InterfaceElementCoreOption("Table (b,B)           : %s", &b.table);
	elements[9]  = new InterfaceElementCoreOption("WaveScale (W)         : %s", &b.waveScale);
	...
	elements[13] = new InterfaceElementCoreOption("Flashlight (s)        : %s", &b.flashlight);
	elements[14] = new InterfaceElementCoreOption("Lights (y/Y)          : %s", &light);
	elements[15] = new InterfaceElementCoreOption("Background (i/I)      : %s", &background);
	...
	// buffer
	CthughaBuffer & b = CthughaBuffer::buffers[ CthughaBuffer::nCurrent ];
	#define O(i)	((InterfaceElementOption*)elements[i])->opt  = \
