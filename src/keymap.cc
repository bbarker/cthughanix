...
Keymap::Keymap(const char * n) :
    next(first), name(n), bindingList(NULL)
{
    first = this;
}
...
void Keymap::readFile(const char * fileName) {
    ...
    printfv(4, "   starting reading keymap file '%s'.\n", fileName);
	...
	if(strncasecmp("#keymap", line, 7) == 0) {
	    ...
	    printfv(5, "      defining keymap '%s'.\n", keymap->name);
	} else {
	    keymap->add(line);
	}
    }
    
    printfv(4, "   finished reading keymap file '%s'.\n", fileName);
    fclose(file);
}
...
Keymap::Binding Keymap::parseBinding(const char * line) {
	...
	Action * A = NULL; int l = -1;				// find longest match
    ...
    printfv(10, "   parsed new keymap entry line '%s'\n", fullLine);
    return b;
}
